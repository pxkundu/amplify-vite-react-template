...
async function createSensors(profile, region) {
  try {
    ...
    sensors.forEach(async (sensor) => {
      ...
      var policyName = "Policy-" + sensor.settings.deviceId;
      var policy = {
      ...
      var fileName =
        CERT_FOLDER + sensor.settings.deviceId + "-certificate.pem.crt";
      sensor.settings.certPath = fileName;
      await fs.writeFile(fileName, certificatePem);

      //save the private key
      fileName = CERT_FOLDER + sensor.settings.deviceId + "-private.pem.key";
      ...
      params = {
        thingTypeName: sensor.settings.deviceType,
      };
      ...
      //create the thing
      params = {
        thingName: sensor.settings.deviceId,
        attributePayload: {
          merge: false,
        },
        thingTypeName: sensor.settings.deviceType,
      ...
      command = new AttachPolicyCommand({
        policyName: policyName,
        target: sensor.settings.certificateArn,
      });
      await iotClient.send(command);

      //attach thing to certificate
      command = new AttachThingPrincipalCommand({
        thingName: sensor.settings.deviceId,
        principal: sensor.settings.certificateArn,
      });
      await iotClient.send(command);
    ...
    sensors.forEach((sensor) => {
      console.log("Thing Name: " + sensor.settings.deviceId);
    });
  } catch (err) {
