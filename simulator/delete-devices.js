...
async function deleteSensors(profile, region) {
  try {
    ...
    sensors.forEach(async (sensor) => {
      ...
      var command = new DetachThingPrincipalCommand({
      ...
      //delete the thing
      command = new DeleteThingCommand({ thingName: sensor.settings.deviceId });
      await iotClient.send(command);
      ...
      command = new UpdateCertificateCommand({
        certificateId: certificateId,
        newStatus: "INACTIVE",
      });
      await iotClient.send(command);

      command = new DeleteCertificateCommand({
        certificateId: certificateId,
        forceDelete: true,
      });
      await iotClient.send(command);
